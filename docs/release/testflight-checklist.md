...
以下项目不能用 Simulator 或 SwiftPM 测试替代。完成后在对应 GitHub issue 中记录设备、系统版本、网络状态、验证结果与任何失败日志；未通过前继续保持功能预览或实验性定位。

### 备份提醒（P1-11，单台真机）

- [ ] 使用 Pro 测试权益，在尚未创建加密备份的设备上启用提醒并允许通知；确认首次通知在下一整分钟送达。
- [ ] 拒绝通知权限后再次尝试启用；确认开关不保持启用状态，并显示可理解的权限提示。
- [ ] 创建一次加密备份后确认不会立即重复提醒，下一次提醒按选定的 7/14/30 天周期重排。
- [ ] 关闭提醒后确认待处理通知已取消；修改周期时确认只保留新的计划。

### Siri 与快捷指令（P1-05，单台真机）

- [ ] 在 App 内保存并测试 API Key 后，用快捷指令分别创建支出、收入和同币种转账；主 App 重开或回到前台后均能看到正确账本、账户余额与交易。
- [ ] 切换当前账本后再运行快捷指令；确认记录写入新账本而非按名称排序的首个账本。
- [ ] 覆盖无 API Key、权限拒绝、无可用分类和账户币种不匹配；每种失败均不写入数据且给出可操作提示。
- [ ] 在 `zh-Hans`、`zh-Hant`、`en`、`ja` 下检查快捷指令参数、结果对话和错误提示。

### 多币种（P1-03，单台真机）

- [ ] 在 Pro 测试权益下建立外币账户并录入外币收支；确认账户选择、账户余额、首页/洞悉汇总和预算不会混入不同币种的原始金额。
- [ ] 在离线状态下使用手动汇率，确认汇率来源和缺失汇率披露明确；恢复网络后不覆盖已固化的历史折算金额。
- [ ] 导出并重新导入 JSON、CSV 与加密备份，确认币种、账户、固定折算金额及汇率历史的保留或明确提示符合数据管理说明。

### iCloud 同步（P1-04，两台真机、同一 Apple ID）

- [ ] 在设备 A 启用同步并重新打开 App 后，再在设备 B 启用；确认两端均打开同一 CloudKit 存储且没有快捷指令写入本地孤立副本。
- [ ] 分别验证新增、编辑和删除交易、账户与分类的双向传播；记录传播延迟与任何冲突结果。
- [ ] 让一台设备离线后创建和编辑数据，再恢复网络；确认数据恢复、删除传播和冲突提示符合预期。
- [ ] 验证未登录 iCloud、账号不可用、容量不足或网络失败时不显示伪造的“同步成功”。
