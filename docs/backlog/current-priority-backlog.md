...
| P1-02 | 分期计划完整状态机 | Partial | 计划详情页现可点按期次切换已还/待还，并通过既有 `updateInstallmentPeriod` 持久化、刷新进度；新增纯状态判定识别逾期期次并用风险色呈现；持久化层新增按期次创建支出、扣减账户余额并幂等绑定 `transactionID`，取消已绑定还款时同步软删除交易并恢复余额；详情页新增多语言“记录还款”入口；持久化层新增提前结清所有未还期次且幂等；详情页新增多语言结清按钮与二次确认；新增跳过期次状态，跳过不产生交易且不再逾期；详情页新增跳过按钮与恢复待还操作；新增待还期次调整到期日，持久化层拒绝已还、已跳过或已关联交易的期次调整，相关文案覆盖四语言并有回归测试；编辑分期计划现改为原地更新，已记录期次、已生成还款交易和未修改首期日时的手动到期日调整都会保留，编辑表单也会保存备注；新增下一未还期次的提前一天系统提醒、提醒入口和计划、期次、记录还款、跳过及提前结清后的重新调度，已无待还期次时会取消旧提醒；提醒入口在权限拒绝、无未来到期日或系统排程失败时会显示四语言错误提示；设置页支持提前 0/1/3/7 天提醒，偏好变化会立即重新调度；仍缺少更细的账单调整 | 建立 installment payment schedule 与交易关联 | 每期还款影响余额和计划进度；支持提前结清；待还期次可调整到期日；用户可开启下一期提醒并选择提前天数 |
