...
| P0B-05 | 分类预算、报销与退款对预算的规则 | Done | `BudgetSetting` 已支持 week/month 和分类/账户范围；`BudgetSpendingCalculator` 会排除已报销支出并以退款抵扣原始支出，持久化与核心计算已有回归测试；从基础总预算首次切换到分类或账户专项预算时会请求 `.openAdvancedBudget`，免费用户保留总预算以及既有专项预算的编辑/删除能力；计划页提供独立“新增专项预算”入口，受 `.openAdvancedBudget` 保护，保存前必须选择分类或账户范围，并阻止同周期内重复范围；Pro 用户可为预算开启“结转未用预算”，仅将上一周期基础预算的正向未用余额带入本期，不抵扣超支且不跨期累计，卡片会区分基础预算、含结转可用额和结转金额；关闭已有结转时需明确确认，配置与旧 JSON 均保持兼容并有四语言/持久化/计算回归测试 | 后续如引入债务递延或多期滚存，需单独定义规则与迁移 | 分类/账户预算可设置和统计；报销/退款按规则排除或抵扣；结转不改变免费基础预算 |
...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；订阅周期支持每日、每个工作日、每周、每月、每月首个工作日、每月末、每季度、每年，以及每隔正整数天/周/月/年的自定义间隔；工作日按系统 Calendar 的非周末日定义，自动记账、手动确认、跳过下一次和提醒共用同一日期计算，周末首次日期会顺延至下一个工作日；每月首个工作日会从年月组件准确定位当月第一天后跳过周末，每月末首次日期会对齐到所选月份的最后一天；Pro 计划页新增只读的计划支出预览，默认未来 30 天且可持久化切换为 90 天或 1 年，按到期日汇总订阅和待还分期，最多展示最近三笔但明确总笔数；仅合计账本币种，异币种计划会明确排除而不进行不安全的跨币种相加，查看或切换范围本身不会创建交易；二月与闰年边界、工作日及月初周末边界、持久化跳过路径、预览计算和四语言文案均有回归测试 | 增加任意每月第 N 个工作日、法定节假日日历等日历型 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
...
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 `currencyCode` 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总、分类分布与预算；洞悉分类详情的总额现直接复用已按目标币种换算的分类分布金额，按日小计也使用交易日期对应的历史汇率重算，缺少历史汇率时明确提示而不再静默忽略不同币种金额；首页各日小计现同样按历史汇率换算，首页与洞悉概览在任一金额无法换算时均显示四语言提示，防止零值或空分类列表被误认为真实汇总；账户新建表单现以运行时默认币种初始化并提供币种选择，选择不同币种统一请求 `.openMultiCurrency` Pro Gate，同币种无阻断；已有账户币种保持只读，避免重释义历史余额与交易；账户总资产现以默认币种汇总，可用历史汇率时换算全部计入资产的账户，缺少汇率的账户会明确显示未计入数量，避免静默漏算；主记账和编辑页现在仅提供与金额币种匹配的账户；没有兼容账户时会说明需先创建对应币种账户，持久化层也会拒绝不匹配写入，且保存失败不再静默伪造成功；已开启自动更新的 Pro 用户会在 App 启动和回到前台时通过既有一小时缓存策略刷新默认币种汇率，免费、过期和关闭开关状态均不发起自动请求；默认币种切换仍免费，但币种页中的在线汇率请求（切换币种与手动刷新）现统一经 `.openAutoExchangeRate` Gate，免费用户不会静默联网；币种页新增免费手动汇率：离线时可输入当前币对汇率并持久化，保存前会明确确认关闭所有自动更新，页面会标识手动来源并允许移除后重新启用自动更新；外币记录在保存或修改时，会自动固化当时已有历史汇率换算出的账本币种金额；后续统计、分类分布和预算均优先使用该固定金额，汇率历史被更正也不会重写这笔账单的已记账结果；该固定金额可随 JSON/CSV 导入导出保留，旧记录则继续按日期回退至历史汇率；CSV 导入缺少币种列时使用注入的默认币种，含外币有效记录时预览会明确说明 CSV 保留原币种但不携带历史汇率快照，并引导完整迁移使用 JSON/加密备份；CSV 预览还会在确认前拦截交易金额与所选账户（包括转账源/目标账户）币种不一致的行，以及记录时固定折算金额币种与目标账本不一致的行，避免导入时或后续统计时悄悄失去固定汇率语义；JSON 和加密备份恢复也会在清空旧数据前校验每笔固定折算金额与快照内账本币种一致，异常时保留现有数据，并在恢复点、JSON 与加密备份入口显示四语言说明；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种；计划页新增订阅和分期使用当前账本币种，编辑已有计划时保留其原币种，并只允许选择同币种账户、默认选择首个可见兼容账户；CSV 导入现在保留导出文件中的交易 `id`（缺失时兼容生成新 ID），并有回归测试；JSON 快照现可选携带历史汇率，导入后恢复 `ExchangeRateService` 缓存并有回归测试；CSV 导入导出现在保留报销与退款关联 ID，以及转账的源/目标账户，并校验转账账户必须存在且不同，均有回归测试；数据管理页 CSV 导出已改为复用该唯一标准导出器，避免旧格式遗漏关联字段或破坏含逗号的备注；仍需继续覆盖更多导入导出场景 | 继续补齐导入导出链路的历史汇率处理与多币种端到端验证 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
| P1-04 | iCloud 同步真实实现或产品降级 | Partial | `RootShellView` 中已使用 SwiftData CloudKit 容器；未注入真实同步闭包时明确失败，不再模拟成功；`SyncSettingsView` 现通过 `CKContainer.accountStatus` 判断账户可用性、集中执行网络/iCloud/蜂窝预检，并在失败时重置进度；手动同步仅显示“已请求”，在未观察到完成事件前不会写入最近同步时间或显示成功，且会阻止重复请求；服务层消费 `NSPersistentCloudKitContainer.eventChangedNotification` 的真实开始、成功、失败生命周期，只有带结束时间且成功的系统事件才更新最近完成时间；由于全局事件无法可靠归因到某次手动请求，UI 明确显示“检测到 iCloud 同步活动已完成”而非笼统宣称本次请求成功；四语言文案与无障碍状态均有回归测试；冲突合并、既有本地数据迁移和真实跨设备验证仍待完成 | 先完成本地→CloudKit 数据迁移与冲突/错误恢复策略，再用两个真实设备、同一 Apple ID 验证新增、编辑、删除与离线恢复；未完成前保持实验性说明 | 用户看得到的同步入口必须真实可用，或在首版隐藏/标记实验 |
...
### 2026-09-08 P1-05 快捷记账与同步存储补充

- App 会将 iCloud 开关镜像到 App Group；App Intent 在独立进程启动时依此选择与主 App 相同的存储模式：本地模式继续使用共享 App Group Store，iCloud 模式改用同一私有 CloudKit 容器，不会再写入孤立的本地副本。
- 快捷指令代码当前随已声明 CloudKit 权限的主 App target 构建。云端首次数据尚未可读时，快捷记账会明确提示“iCloud 数据正在准备中”，而不是误导用户去配置分类；并会安全拒绝写入。后续必须在真机、同一 Apple ID 下验证快捷指令写入、主界面回显与离线恢复。

### 2026-09-08 Pro 加密备份提醒补充

- 本地备份页新增受既有加密备份 Pro Gate 保护的定期提醒，可选每 7/14/30 天。提醒只通知用户手动创建加密备份，不保存密码、不自动导出或上传数据；每次成功创建备份后会按新时间重排下一次提醒。通知权限被拒绝或系统排程失败时会给出四语言可读提示。
...
| P2-07 | 高级洞悉模块排序/隐藏 | Partial | Pro 洞悉页现可在受 `.openAdvancedInsights` 保护的自定义面板中显示/隐藏收入与支出分类分布，并使用箭头调整模块顺序；基础收支汇总始终保留，偏好会持久化；另支持按账户筛选当前范围、上一等长周期及分类分布，转账的转出/转入账户都会匹配，已删除账户的旧筛选会自动回退到全部账户；自定义范围还会逐日展示收入/支出趋势，空日期保持可见，外币优先使用记账时固化金额后再回退历史汇率，缺率不展示错误趋势；高级选项新增 CSV 洞悉报告导出，文件只含当前范围与账户筛选下的支出、收入、结余、记账次数汇总，金额使用可计算的原始十进制数值，账户名含逗号时正确转义，不会暴露交易明细 | 后续增加专项统计 |
...
| PRO-03 | FeatureGate 能力闸口 | Partial | 新增统一 paywall presenter；新建账本、账户、订阅、分期均按免费额度拦截，自动汇率、iCloud 启用和加密备份创建按能力拦截；计划页未来 30 天订阅与待还分期支出预览现通过 `.openAdvancedPlans` Gate，免费卡片会说明 Pro 门槛并进入对应升级页，Pro 端仅预览不产生交易且明确异币种排除；分类/账户专项预算首次从基础预算切换时接入 `.openAdvancedBudget`，降级用户仍可维护已有专项预算；洞悉页自定义日期范围现接入 `.openAdvancedInsights`，日期归一化、完整结束日和免费/Pro Gate 均有回归覆盖；自定义范围现在会自动计算相同自然日数的上一期，展示支出、收入和结余的变化并复用历史汇率缺失与隐私金额策略，跨夏令时不会因固定秒数计算错位；Pro 用户还可显示/隐藏收入与支出分类分布，并用箭头排序，基础汇总不会被隐藏且偏好会持久化；按账户筛选会一致作用于当前区间、上一期对比与分类分布，转账双侧均匹配，失效账户筛选自动回退；自定义范围逐日趋势会保留无交易日期、记账时固定折算金额及历史汇率失败披露；CSV 预览页的新建映射模板现接入 `.openAdvancedImportExport`，已有模板仍可加载和删除；暖色主题的新选择现接入 `.openPremiumThemes`，默认主题与系统外观模式保持免费，降级后已选暖色主题不会被强制改写；批量编辑现接入 `.openBatchEdit`，仅允许同一收支类型的非转账记录批量调整分类，执行前会展示目标分类与记录数确认，成功后提供五秒原子撤销；金额、日期、账户、备注和余额均不会被修改；App 内 AI 快速记账入口会先经 `.openAIRecord`，免费用户进入上下文 paywall，Pro 无 Key 用户被引导直达 AI 配置，已配置用户在服务商披露后只生成可编辑草稿；快捷指令 URL 复用同一 gate；既有数据编辑、删除、恢复及基础导出未锁定 | 为 AI 增加真实设备/网络失败验收，并完成其余收费能力的端到端核验 | 所有已实现收费能力只通过统一 gate 判断 |
| PRO-04 | Paywall 与会员状态页 | Partial | 设置页状态卡与权益页已连接实时会员状态；价格由 StoreKit 返回，购买/待批准/验签失败/恢复/续订管理/到期状态与上下文升级页均已接入，年付卡仅在已加载月付与年付实际价格、且年付低于十二个月月付总价时显示计算所得的节省比例；文案覆盖四种运行时语言；权益页现以 7 张轻量 SVG Banner 展示无限账本/账户、订阅与分期一处掌控、多币种与汇率、跨设备同步、AI 快速记账、高级主题与加密备份，订阅/分期合并避免重复叙事；订阅与分期 Banner 已改为日历、循环扣费和还款进度的组合场景；多币种与跨设备同步在 Banner 和权益对比中统一明确标为“功能预览 · 暂未包含在 Pro 中”，避免被误售；AI 快速记账已改为 Pro + 自带 API Key 的已售权益，并在入口中保留服务商披露与草稿审核；未来 30 天计划支出预览仍已交付但不再占用主 Banner；首页摘要现提供独立 Pro 权限：可调整支出、收入、结余、记账次数四卡顺序，免费版仍保持原有收支摘要，权益对比已明确列出；分页控件、套餐 CTA 和权益对比均补充动态字体/无障碍语义与主题对比度处理；且只有用户协议与隐私政策均配置为合法 HTTPS 地址时才允许启动购买 | 配置用户协议与隐私协议正式链接；完成多币种、真实双设备 iCloud、AI 真机失败分支与 Sandbox 购买验收 | 设置页可进入会员页；受限功能可弹上下文 paywall；预览能力不被误售 |
