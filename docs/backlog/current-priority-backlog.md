...
| P0B-06 | 搜索/筛选/编辑删除闭环验收 | Partial | 已有 `TransactionSearchView`、交易编辑删除撤销路径；仍需与 PRD 筛选维度对齐 | 补齐时间、类型、分类、账户、金额、关键字组合筛选；编辑/删除后统计和余额刷新 | P0 UI Test 覆盖新增、搜索、筛选、编辑、删除、撤销 |
| P0B-07 | 转账与账户统计边界 | Partial | `TransactionSummaryTests` 覆盖转账不计入收支；SwiftData 更新转账测试仍失败 | 统一 domain 与 persistence 的 transfer balance adjustment；补跨账户/跨币种边界 | 转账只移动资产，不污染收入/支出；账户余额更新可回归 |
...
| P1-02 | 分期计划完整状态机 | Partial | `InstallmentPlan`、计划页已有进度展示；缺少还款记录绑定、提前结清、逾期/跳过等状态 | 建立 installment payment schedule 与交易关联 | 每期还款影响余额和计划进度；支持提前结清 |
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 currencyCode 字段已存在；`TransactionService` 仍有 `"CNY"` 硬编码 | 将默认币种注入 AI/快捷记账/交易服务；统计支持主币种换算与汇率快照 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
