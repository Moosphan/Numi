...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅的入口和回归测试，相关文案覆盖四语言；仍缺少通知提醒和用户确认扣费 | 增加 recurring rule、通知提醒、用户确认扣费 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
