...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；订阅周期新增按季度扣费，表单、自动记账、跳过下一次和提醒均复用现有周期模型，且有四语言与回归测试；仍缺少自定义间隔规则 | 增加自定义间隔 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
| P1-02 | 分期计划完整状态机 | Partial | 计划详情页现可点按期次切换已还/待还，并通过既有 `updateInstallmentPeriod` 持久化、刷新进度；新增纯状态判定识别逾期期次并用风险色呈现；持久化层新增按期次创建支出、扣减账户余额并幂等绑定 `transactionID`，取消已绑定还款时同步软删除交易并恢复余额；详情页新增多语言“记录还款”入口；持久化层新增提前结清所有未还期次且幂等；详情页新增多语言结清按钮与二次确认；新增跳过期次状态，跳过不产生交易且不再逾期；详情页新增跳过按钮与恢复待还操作；新增待还期次调整到期日，持久化层拒绝已还、已跳过或已关联交易的期次调整，相关文案覆盖四语言并有回归测试；新增下一未还期次的提前一天系统提醒、提醒入口和计划、期次、记录还款、跳过及提前结清后的重新调度，已无待还期次时会取消旧提醒；设置页支持提前 0/1/3/7 天提醒，偏好变化会立即重新调度；仍缺少更细的账单调整 | 建立 installment payment schedule 与交易关联；补充分期提醒的通知授权/失败反馈 | 每期还款影响余额和计划进度；支持提前结清；待还期次可调整到期日；用户可开启下一期提醒并选择提前天数 |
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 `currencyCode` 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总、分类分布与预算；CSV 导入缺少币种列时使用注入的默认币种；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种；CSV 导入现在保留导出文件中的交易 `id`（缺失时兼容生成新 ID），并有回归测试；JSON 快照现可选携带历史汇率，导入后恢复 `ExchangeRateService` 缓存并有回归测试；CSV 导入导出现在保留报销与退款关联 ID，以及转账的源/目标账户，并校验转账账户必须存在且不同，均有回归测试；仍需继续覆盖更多导入导出场景 | 继续补齐导入导出链路的历史汇率处理与多币种端到端验证 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
