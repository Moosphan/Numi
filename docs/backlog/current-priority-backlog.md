...
| P2-07 | 高级洞悉模块排序/隐藏 | Partial | Pro 洞悉页现可在受 `.openAdvancedInsights` 保护的自定义面板中显示/隐藏收入与支出分类分布，并使用箭头调整模块顺序；基础收支汇总始终保留，偏好会持久化；另支持按账户筛选当前范围、上一等长周期及分类分布，转账的转出/转入账户都会匹配，已删除账户的旧筛选会自动回退到全部账户；自定义范围还会逐日展示收入/支出趋势，空日期保持可见，外币优先使用记账时固化金额后再回退历史汇率，缺率不展示错误趋势 | 后续增加专项统计与高级报告 |
