...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；订阅周期支持每日、每个工作日、每周、每月、每月首个工作日、每月末、每季度、每年，以及每隔正整数天/周/月/年的自定义间隔；工作日按系统 Calendar 的非周末日定义，自动记账、手动确认、跳过下一次和提醒共用同一日期计算，周末首次日期会顺延至下一个工作日；每月首个工作日会从年月组件准确定位当月第一天后跳过周末，每月末首次日期会对齐到所选月份的最后一天；二月与闰年边界、工作日及月初周末边界、持久化跳过路径和四语言文案均有回归测试 | 增加任意每月第 N 个工作日、法定节假日日历等日历型 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
| P1-02 | 分期计划完整状态机 | Partial | 计划详情页现可点按期次切换已还/待还，并通过既有 `updateInstallmentPeriod` 持久化、刷新进度；新增纯状态判定识别逾期期次并用风险色呈现；持久化层新增按期次创建支出、扣减账户余额并幂等绑定 `transactionID`，取消已绑定还款时同步软删除交易并恢复余额；详情页新增多语言“记录还款”入口，默认使用到期日但可选择实际还款入账日期；持久化层新增提前结清所有未还期次且幂等；详情页新增多语言结清按钮与二次确认；新增跳过期次状态，跳过不产生交易且不再逾期；详情页新增跳过按钮与恢复待还操作；新增待还期次调整到期日，持久化层拒绝已还、已跳过或已关联交易的期次调整，相关文案覆盖四语言并有回归测试；编辑分期计划现改为原地更新，已记录期次、已生成还款交易和未修改首期日时的手动到期日调整都会保留，编辑表单也会保存备注；新增下一未还期次的提前一天系统提醒、提醒入口和计划、期次、记录还款、跳过及提前结清后的重新调度，已无待还期次时会取消旧提醒；提醒入口在权限拒绝、无未来到期日或系统排程失败时会显示四语言错误提示；设置页支持提前 0/1/3/7 天提醒，偏好变化会立即重新调度；仍缺少更细的账单调整 | 建立 installment payment schedule 与交易关联 | 每期还款影响余额和计划进度；支持提前结清；待还期次可调整到期日；用户可开启下一期提醒并选择提前天数 |
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 `currencyCode` 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总、分类分布与预算；CSV 导入缺少币种列时使用注入的默认币种；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种；计划页新增订阅和分期使用当前账本币种，编辑已有计划时保留其原币种，并只允许选择同币种账户、默认选择首个可见兼容账户；CSV 导入现在保留导出文件中的交易 `id`（缺失时兼容生成新 ID），并有回归测试；JSON 快照现可选携带历史汇率，导入后恢复 `ExchangeRateService` 缓存并有回归测试；CSV 导入导出现在保留报销与退款关联 ID，以及转账的源/目标账户，并校验转账账户必须存在且不同，均有回归测试；数据管理页 CSV 导出已改为复用该唯一标准导出器，避免旧格式遗漏关联字段或破坏含逗号的备注；仍需继续覆盖更多导入导出场景 | 继续补齐导入导出链路的历史汇率处理与多币种端到端验证 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
| P1-04 | iCloud 同步真实实现或产品降级 | Partial | `RootShellView` 中已使用 SwiftData CloudKit 容器；未注入真实同步闭包时明确失败，不再模拟成功；`SyncSettingsView` 现通过 `CKContainer.accountStatus` 判断账户可用性、集中执行网络/iCloud/蜂窝预检，并在失败时重置进度；服务层新增同步防重入保护，避免生命周期和手动触发造成并发同步状态错乱，并有回归测试；冲突合并与跨设备验证仍待完成 | 明确首版是否发布同步；若保留则实现冲突合并、错误恢复、跨设备验证；若不保留则隐藏入口 | 用户看得到的同步入口必须真实可用，或在首版隐藏/标记实验 |
| P1-05 | App Intents / Siri 记账产品化 | Partial | `NumiIntents/RecordTransactionIntent.swift` 与 shortcuts provider 已存在；`TransactionService` 现会解析转账目标账户、校验同币种且更新双方余额；Intent 标题、描述、参数、成功/失败 dialog 已改用四语言资源；App Group/SwiftData 初始化失败会返回本地化错误而不再触发扩展崩溃；成功提示现使用实际账本币种格式，不再固定显示人民币符号；快捷指令现在遵循 App 中保存的 Claude/Qwen/DeepSeek 提供商设置，并在首选 Key 缺失时按稳定顺序回退，相关策略有回归测试；仍需补充真实 Intent 端到端验证 | 接入真实 store、默认账本/账户/币种、本地化 dialog、失败恢复 | Siri/快捷指令可稳定创建支出/收入/转账 |
