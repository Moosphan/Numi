...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；订阅周期支持每日、每周、每月、每季度、每年，以及每隔正整数天/周/月/年的自定义间隔；自动记账、手动确认、跳过下一次和提醒共用同一日期计算，间隔数据可持久化，相关文案覆盖四语言并有回归测试；仍缺少按工作日、月末等日历规则 | 增加工作日/月末等日历型 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
| P1-02 | 分期计划完整状态机 | Partial | 计划详情页现可点按期次切换已还/待还，并通过既有 `updateInstallmentPeriod` 持久化、刷新进度；新增纯状态判定识别逾期期次并用风险色呈现；持久化层新增按期次创建支出、扣减账户余额并幂等绑定 `transactionID`，取消已绑定还款时同步软删除交易并恢复余额；详情页新增多语言“记录还款”入口；持久化层新增提前结清所有未还期次且幂等；详情页新增多语言结清按钮与二次确认；新增跳过期次状态，跳过不产生交易且不再逾期；详情页新增跳过按钮与恢复待还操作；新增待还期次调整到期日，持久化层拒绝已还、已跳过或已关联交易的期次调整，相关文案覆盖四语言并有回归测试；新增下一未还期次的提前一天系统提醒、提醒入口和计划、期次、记录还款、跳过及提前结清后的重新调度，已无待还期次时会取消旧提醒；提醒入口在通知权限被拒绝或请求失败时会显示四语言错误提示；设置页支持提前 0/1/3/7 天提醒，偏好变化会立即重新调度；仍缺少更细的账单调整 | 建立 installment payment schedule 与交易关联；补充分期提醒排程失败反馈 | 每期还款影响余额和计划进度；支持提前结清；待还期次可调整到期日；用户可开启下一期提醒并选择提前天数 |
