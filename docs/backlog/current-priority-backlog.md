...
| P1-02 | 分期计划完整状态机 | Partial | 计划详情页现可点按期次切换已还/待还，并通过既有 `updateInstallmentPeriod` 持久化、刷新进度；新增纯状态判定识别逾期期次并用风险色呈现；持久化层新增按期次创建支出、扣减账户余额并幂等绑定 `transactionID`，取消已绑定还款时同步软删除交易并恢复余额；详情页新增多语言“记录还款”入口；持久化层新增提前结清所有未还期次且幂等；详情页新增多语言结清按钮与二次确认；新增跳过期次状态，跳过不产生交易且不再逾期；仍缺少详情页跳过入口 | 建立 installment payment schedule 与交易关联 | 每期还款影响余额和计划进度；支持提前结清 |
