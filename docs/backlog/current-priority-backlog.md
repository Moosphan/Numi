...
| PRO-04 | Paywall 与会员状态页 | Partial | 设置页状态卡与权益页已连接实时会员状态；价格由 StoreKit 返回，购买/待批准/验签失败/恢复/续订管理/到期状态与上下文升级页均已接入，文案覆盖四种运行时语言；权益页现仅宣传 V1 已发布的无限账本/账户、更多订阅/分期与加密备份，且只有用户协议与隐私政策均配置为合法 HTTPS 地址时才允许启动购买 | 配置用户协议与隐私协议正式链接；用 Sandbox 完成真机或模拟器端到端支付验证 | 设置页可进入会员页；受限功能可弹上下文 paywall |
