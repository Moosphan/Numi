...
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 `currencyCode` 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总、分类分布与预算；CSV 导入缺少币种列时使用注入的默认币种；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种；CSV 导入现在保留导出文件中的交易 `id`（缺失时兼容生成新 ID），并有回归测试；JSON 快照现可选携带历史汇率，导入后恢复 `ExchangeRateService` 缓存并有回归测试；仍需继续覆盖更多导入导出场景 | 继续补齐导入导出链路的历史汇率处理与多币种端到端验证 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
