...
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 `currencyCode` 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总、分类分布与预算；账户新建表单现以运行时默认币种初始化并提供币种选择，选择不同币种统一请求 `.openMultiCurrency` Pro Gate，同币种无阻断；已有账户币种保持只读，避免重释义历史余额与交易；已开启自动更新的 Pro 用户会在 App 启动和回到前台时通过既有一小时缓存策略刷新默认币种汇率，免费、过期和关闭开关状态均不发起自动请求；CSV 导入缺少币种列时使用注入的默认币种；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种；计划页新增订阅和分期使用当前账本币种，编辑已有计划时保留其原币种，并只允许选择同币种账户、默认选择首个可见兼容账户；CSV 导入现在保留导出文件中的交易 `id`（缺失时兼容生成新 ID），并有回归测试；JSON 快照现可选携带历史汇率，导入后恢复 `ExchangeRateService` 缓存并有回归测试；CSV 导入导出现在保留报销与退款关联 ID，以及转账的源/目标账户，并校验转账账户必须存在且不同，均有回归测试；数据管理页 CSV 导出已改为复用该唯一标准导出器，避免旧格式遗漏关联字段或破坏含逗号的备注；仍需继续覆盖更多导入导出场景 | 继续补齐导入导出链路的历史汇率处理与多币种端到端验证 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
