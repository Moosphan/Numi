...
| PRO-04 | Paywall 与会员状态页 | Partial | 设置页状态卡与权益页已连接实时会员状态；价格由 StoreKit 返回，购买/待批准/验签失败/恢复/续订管理/到期状态与上下文升级页均已接入，年付卡仅在已加载月付与年付实际价格、且年付低于十二个月月付总价时显示计算所得的节省比例；文案覆盖四种运行时语言；权益页现以 7 张轻量 SVG Banner 展示无限账本/账户、订阅与分期一处掌控、多币种与汇率、跨设备同步、AI 快速记账、高级主题与加密备份，订阅/分期合并避免重复叙事；订阅与分期 Banner 已改为日历、循环扣费和还款进度的组合场景；多币种、跨设备同步与 AI 快速记账在 Banner 和权益对比中统一明确标为“功能预览 · 暂未包含在 Pro 中”，避免被误售；AI 预览可保留自带 API Key、服务商披露与草稿审核说明，但在正式策略和真机验收前不构成购买承诺；未来 30 天计划支出预览仍已交付但不再占用主 Banner；分页控件、套餐 CTA 和权益对比均补充动态字体/无障碍语义与主题对比度处理；且只有用户协议与隐私政策均配置为合法 HTTPS 地址时才允许启动购买 | 配置用户协议与隐私协议正式链接；完成多币种、真实双设备 iCloud、AI 策略与真机失败分支、Sandbox 购买验收 | 设置页可进入会员页；受限功能可弹上下文 paywall；预览能力不被误售 |
