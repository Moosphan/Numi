...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；订阅周期支持每日、每个工作日、每周、每月、每月首个工作日、每月末、每季度、每年，以及每隔正整数天/周/月/年的自定义间隔；工作日按系统 Calendar 的非周末日定义，自动记账、手动确认、跳过下一次和提醒共用同一日期计算，周末首次日期会顺延至下一个工作日；每月首个工作日会从年月组件准确定位当月第一天后跳过周末，每月末首次日期会对齐到所选月份的最后一天；Pro 计划页新增只读的未来 30 天支出预览，按到期日汇总订阅和待还分期，最多展示最近三笔但明确总笔数；仅合计账本币种，异币种计划会明确排除而不进行不安全的跨币种相加，查看本身不会创建交易；二月与闰年边界、工作日及月初周末边界、持久化跳过路径、预览计算和四语言文案均有回归测试 | 增加任意每月第 N 个工作日、法定节假日日历等日历型 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
