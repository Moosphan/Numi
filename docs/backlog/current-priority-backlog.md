...
| PRO-03 | FeatureGate 能力闸口 | Partial | 新增统一 paywall presenter；新建账本、账户、订阅、分期均按免费额度拦截，自动汇率、iCloud 启用和加密备份创建按能力拦截；分类/账户专项预算首次从基础预算切换时接入 `.openAdvancedBudget`，降级用户仍可维护已有专项预算；洞悉页自定义日期范围现接入 `.openAdvancedInsights`，日期归一化、完整结束日和免费/Pro Gate 均有回归覆盖；自定义范围现在会自动计算相同自然日数的上一期，展示支出、收入和结余的变化并复用历史汇率缺失与隐私金额策略，跨夏令时不会因固定秒数计算错位；CSV 预览页的新建映射模板现接入 `.openAdvancedImportExport`，已有模板仍可加载和删除；暖色主题的新选择现接入 `.openPremiumThemes`，默认主题与系统外观模式保持免费，降级后已选暖色主题不会被强制改写；批量编辑现接入 `.openBatchEdit`，仅允许同一收支类型的非转账记录批量调整分类，执行前会展示目标分类与记录数确认，成功后提供五秒原子撤销；金额、日期、账户、备注和余额均不会被修改；既有数据编辑、删除、恢复及基础导出未锁定 | AI 能力补齐隐私告知、可校正草稿与成本策略后，再接入同一 gate | 所有已实现收费能力只通过统一 gate 判断 |
