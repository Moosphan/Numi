...
| ID | Backlog | 当前状态 | 证据 | 下一步 | 验收 |
| --- | --- | --- | --- | --- | --- |
| P1-01 | 订阅/循环记账自动化 | Partial | 已支持启用订阅到期计算、到期生成支出、推进下次扣费日期与幂等处理；App 启动和回到前台会触发处理；新增跳过下一次扣费、暂停/恢复订阅、系统通知授权与下一次提醒调度，以及到期后确认扣费的一次性记录入口，相关文案覆盖四语言并有回归测试；设置页现支持订阅提醒提前 0/1/3/7 天并在偏好变化后重新调度；仍缺少更完整的 recurring rule 配置 | 增加 recurring rule 配置 | 到期订阅可提醒并生成交易，用户可跳过或确认 |
