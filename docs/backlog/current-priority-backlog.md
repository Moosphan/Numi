...
| P1-04 | iCloud 同步真实实现或产品降级 | Partial | `RootShellView` 中已使用 SwiftData CloudKit 容器；未注入真实同步闭包时明确失败，不再模拟成功；`SyncSettingsView` 现通过 `CKContainer.accountStatus` 判断账户可用性、集中执行网络/iCloud/蜂窝预检，并在失败时重置进度；冲突合并与跨设备验证仍待完成 | 明确首版是否发布同步；若保留则实现冲突合并、错误恢复、跨设备验证；若不保留则隐藏入口 | 用户看得到的同步入口必须真实可用，或在首版隐藏/标记实验 |
