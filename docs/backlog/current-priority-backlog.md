...
| P1-03 | 多币种全链路 | Partial | 币种管理、汇率服务和 currencyCode 字段已存在；已新增按日期持久化汇率快照、主币种金额转换，并接入首页/洞悉汇总与分类分布；App 内 AI 记账及 `TransactionService` 快捷记账路径已跟随当前账本币种，仍缺少预算与导入服务的默认币种注入 | 将默认币种继续注入导入服务；预算支持主币种换算并继续覆盖导入导出链路 | 新增、统计、预算、导入导出、AI 记账都尊重默认币种和历史汇率 |
