...
- 开启“iCloud 同步”后，App 会使用当前 Apple ID 的私有 CloudKit 容器同步数据；该开关默认为关闭。首次开启时会先保存当前设备的完整迁移快照，并提示重新打开 App，以便安全切换到 CloudKit 存储。
- 若重新打开后两端都有数据，会显示交易、账户和分期数量，供你选择保留“本机完整账本”或“iCloud 完整账本”；不会按单条记录自动混合，避免重复交易或破坏账户余额。确认前可选择“暂不迁移”。每次确认会在本机保留两端恢复副本。
- iCloud 同步不是独立备份。真实多设备验证仍在持续完善，重要数据请保留自行导出的加密备份。
