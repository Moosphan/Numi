...
/// 基于 DeepSeek API 的账单解析器
public final class DeepSeekTransactionParser: TransactionLLMService, @unchecked Sendable {
    private let apiKey: String
    private let session: URLSession
    private let model: String

    public init(apiKey: String, model: String = "deepseek-chat", session: URLSession = .shared) {
        self.apiKey = apiKey
        self.model = model
    ...
    private func callLLM(prompt: String) async throws -> ParsedTransactionDTO {
        ...
        let dsResp = try JSONDecoder().decode(DeepSeekResponse.self, from: data)
        guard let content = dsResp.choices.first?.message.content else {
