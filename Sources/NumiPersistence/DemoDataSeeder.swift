...
public enum DemoDataSeeder {
    ...
    private static func seedShowcase(into store: SwiftDataBookkeepingStore) throws {
        ...
        let now = Date()
        try createExpense(store, amount: "18.00", categoryID: food.id, accountID: cash.id, note: "早餐 豆浆油条", daysAgo: 0, now: now)
        try createExpense(store, amount: "32.00", categoryID: food.id, accountID: card.id, note: "午餐 轻食沙拉", daysAgo: 0, now: now)
        try createExpense(store, amount: "24.50", categoryID: food.id, accountID: alipay.id, note: "下午咖啡", daysAgo: 1, now: now)
        try createExpense(store, amount: "14.00", categoryID: transport.id, accountID: wechat.id, note: "地铁通勤", daysAgo: 1, now: now)
        try createExpense(store, amount: "399.00", categoryID: shopping.id, accountID: card.id, note: "收纳补货", daysAgo: 2, now: now)
        try createExpense(store, amount: "980.00", categoryID: housing.id, accountID: bank.id, note: "房租分摊", daysAgo: 4, now: now)
        try createExpense(store, amount: "68.00", categoryID: subscription.id, accountID: credit.id, note: "iCloud+ 年费分摊", daysAgo: 5, now: now)

        try createIncome(store, amount: "9200.00", categoryID: salary.id, accountID: card.id, note: "六月工资", daysAgo: 8, now: now)
        try createIncome(store, amount: "600.00", categoryID: sideJob.id, accountID: alipay.id, note: "周末拍摄", daysAgo: 6, now: now)
        try createIncome(store, amount: "88.00", categoryID: refund.id, accountID: wechat.id, note: "外卖退款", daysAgo: 3, now: now)
        ...
        try createMarkerTransaction(store, accountID: cash.id, note: showcaseMarker, now: now)
    }
    ...
    private static func seedPlansFocus(into store: SwiftDataBookkeepingStore) throws {
        ...
        let now = Date()
        try createIncome(store, amount: "8600.00", categoryID: salary.id, accountID: bank.id, note: "月收入", daysAgo: 9, now: now)
        try createExpense(store, amount: "980.00", categoryID: food.id, accountID: bank.id, note: "餐饮累计", daysAgo: 1, now: now)
        try createExpense(store, amount: "1280.00", categoryID: shopping.id, accountID: bank.id, note: "家居采购", daysAgo: 2, now: now)
        try createExpense(store, amount: "360.00", categoryID: transport.id, accountID: bank.id, note: "打车与通勤", daysAgo: 3, now: now)
        ...
        try createMarkerTransaction(store, accountID: bank.id, note: plansMarker, now: now)
    }
    ...
    private static func seedScreenshotShowcase(into store: SwiftDataBookkeepingStore) throws {
        ...
        let now = Date()
        try createExpense(store, amount: "28.00", categoryID: food.id, accountID: cash.id, note: "早餐 三明治和美式", daysAgo: 0, now: now)
        try createExpense(store, amount: "76.50", categoryID: food.id, accountID: alipay.id, note: "团队午餐", daysAgo: 1, now: now)
        try createExpense(store, amount: "32.00", categoryID: food.id, accountID: wechat.id, note: "夜间水果补给", daysAgo: 3, now: now)
        try createExpense(store, amount: "46.00", categoryID: transport.id, accountID: wechat.id, note: "周内通勤", daysAgo: 2, now: now)
        try createExpense(store, amount: "410.00", categoryID: shopping.id, accountID: credit.id, note: "书桌收纳整理", daysAgo: 4, now: now)
        try createExpense(store, amount: "114.00", categoryID: entertainment.id, accountID: alipay.id, note: "周末市集和展览", daysAgo: 5, now: now)
        try createExpense(store, amount: "960.00", categoryID: housing.id, accountID: mainBank.id, note: "本月房租分摊", daysAgo: 7, now: now)
        try createExpense(store, amount: "1400.00", categoryID: shopping.id, accountID: mainBank.id, note: "年度设备维护", daysAgo: 9, now: now)

        try createIncome(store, amount: "9800.00", categoryID: salary.id, accountID: mainBank.id, note: "六月工资", daysAgo: 10, now: now)
        try createIncome(store, amount: "1580.00", categoryID: sideJob.id, accountID: alipay.id, note: "品牌插画项目", daysAgo: 6, now: now)
        try createIncome(store, amount: "350.00", categoryID: refund.id, accountID: wechat.id, note: "差旅报销", daysAgo: 2, now: now)
        ...
        try createMarkerTransaction(store, accountID: cash.id, note: screenshotMarker, now: now)
    }
